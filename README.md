# Gamepad-Core Unity Bridge

Gamepad-Core Unity Bridge is a native C++ library that exposes the Gamepad-Core functionality through a high-level C-compatible API.

The exported API is designed to be consumed by Unity through C# P/Invoke, but it can also be used from other languages that support native dynamic libraries and function-pointer callbacks. The bridge keeps the platform-specific implementation outside the core library and forwards device operations to callbacks supplied by the host application.

## Features

- Device registry integration for Unity or another host engine.
...
The build produces the `GamepadCoreUnity` shared library.

## Native API

The public API is declared in [`gamepad_unity_api.h`](gamepad_unity_api.h). The host application should:

1. Register the platform callbacks with `GCU_InitializePlatformBridge`.
2. Register the device registry callbacks with `GCU_InitializeDeviceRegistryPolicy`.
3. Call `GCU_DiscoverDevices` from the host application's update loop.
...
- `gamepad_unity_api.h` — exported API and callback declarations.
