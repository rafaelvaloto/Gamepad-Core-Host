...
if [ ! -d "${SOURCE_DIR}" ]; then
	printf "${RED}Error: source directory %s not found${NC}\n" "${SOURCE_DIR}" >&2
	exit 1
fi

# Format only the project's source files. The third-party dependency is kept
# outside this scope so its formatting remains controlled by its own project.
FIND_ARGS=("${SOURCE_DIR}" \( -name "*.h" -o -name "*.hpp" -o -name "*.c" -o -name "*.cc" -o -name "*.cpp" -o -name "*.cxx" \))
